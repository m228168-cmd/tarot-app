...
以下是一段影片的逐字稿候選片段，每段約 30 秒。請選出最能讓人「停下來看完」的片段，組合成 30-60 秒的精華。

你的目標是選出讓人忍不住分享的內容。選擇標準（依重要性排序）：
1. 有趣、好笑、或讓人意想不到的反轉（最重要！短影音要好玩）
2. 能引動情緒：震驚、感動、憤怒、共鳴、好奇
3. 有金句或一句讓人記住的話
4. 完整的概念（有頭有尾，不要切到一半）
5. 節奏快、內容密度高（不要選廢話多的段落）

重要：時長 30-60 秒都可以，由你判斷這段內容用多長效果最好。
- 如果一段內容本身完整且節奏緊湊，30 秒就夠
- 如果有鋪陳→轉折→收尾的完整弧線，可以拉到 50-60 秒
- 不要為了湊時長硬加無聊段落，也不要為了縮短而砍掉關鍵轉折
- 可以選 1 段也可以選多段，以「看完最過癮」為標準

請回傳 JSON 格式：
{
  "ranges": [
    { "start": 數字（秒）, "end": 數字（秒）, "reason": "為什麼這段會吸引人看完" }
  ],
  "totalDuration": 數字（秒）,
  "hook": "短影音標題（繁體中文，最多12字，要有衝擊力、像標題不像描述，不要問句，不要句子斷一半）"
}
